...
//  标识 react 节点，具体创建fiberRoot的方法
function legacyRenderSubtreeIntoContainer(
  parentComponent: ?React$Component<any, any>,
  children: ReactNodeList,
  container: DOMContainer,
  forceHydrate: boolean,  // 是否进行强制调和
  callback: ?Function,
) {
  if (__DEV__) {
  ...
  //  _ReactSyncRoot 是啥 ??  _ReactSyncRoot 是一个接口 {  ..., _internalRoot: FiberRoot }
  //  其中 _internalRoot 属性为值一个叫 FiberRoot 的类型 我们可以查看下 它的类型 来自 react-reconciler/src/ReactFiberRoot.js
  //  如果 container 元素上有 _reactRootContainer 属性 则 root 是有初始值的 也就是本容器本身已经是一个fiberRoot了
  //  否则 则需要挂在一个 _reactRootContainer 使之变成一个 fiberRoot, 这层意思也就是标识 react 节点。
  ...
  if (!root) {
    ...
    // 首次创建fiberRoot 对象，也就是模板首次渲染，不应先渲染view 再把state填回去，
    // 而是直接把state里面的数据更新到页面 更早到达用户面前
    unbatchedUpdates(() => {
    ...
    // Update
       // 非首次创建 fiberRoot 对象，意味着这只是一次 state 修改触发的页面更新 等待 view
    // 渲染完 再统一处理我们 state 的状态,  批量处理, 防止多次更新。这里涉及到 setState
    // 优先级的问题，是通过一个时间字段去标识的，待会儿说。
    updateContainer(children, fiberRoot, parentComponent, callback);
  }
...
const ReactDOM: Object = {
  ...
  // 新增了 hydrate 使用服务端渲染和本地 DOM 进行调和
  ...
  /**
   * render 到页面的方法
   * @param  {[type]} element:   React$Element<any> [组件节点]
   * @param  {[type]} container: DOMContainer       [节点插入的容器]
   * @param  {[type]} callback:  ?Function          [回调函数, 可选]
   * @return {[type]}            [legacyRenderSubtreeIntoContainer方法, wtf ? 这方法是把当前的tree渲染到container里面取]
   */
  render(
